...
async function create(name, genre, website, recordLabel, bandMembers, yearFormed) {
    ...
    if (!(website.includes("http://www.")) || !(website.includes(".com"))) {
        throw "ERROR: MUST INCLUDE HTTP://WWW. & .COM in WEBSITE";
    }
    if ((website.includes("http://www.") && (website.includes(".com")))) {
...
async function update(id, name, genre, website, recordLabel, bandMembers, yearFormed) { 
    ...
    for (let i = 0; i < genre.length; i++){
        if (typeof genre[i] !== 'string' && genre.length === 0) {
            throw "ERROR: MUST NOT BE AN EMPTY STRING!"
        }
    }
    for (let i = 0; i < bandMembers.length; i++){
        if (typeof bandMembers[i] !== 'string' && bandMembers.length === 0) {
            throw "ERROR: MUST NOT BE AN EMPTY STRING!"
        }
    }
    // error check #5
    ...
    const updateInfo = await bandsCollection.findOne(
        {_id: ObjectId(id)}
    )
    if (!updateInfo) {
    ...
    let bandsUpdateInfo = {
         ...
         bandMembers: bandMembers,
         yearFormed: yearFormed,
         albums: updateInfo.albums,
         overallRating: updateInfo.overallRating
    }
    const updateI = await bandsCollection.updateOne(
        {_id: ObjectId(id)},
        {$set: bandsUpdateInfo}
    ) 
